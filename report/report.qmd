...
illustration1: "./images/hist_qf2.png"
...
The image compression is based on redundancies, both statistical and psychovisual.

- Statistical redundancies. In fact, there is a lot of correlation between pixels or colors of an image. Theses correlations, both spatial and spectral, introduce redundancies that can be used to compress and store intelligently data of image.
- Psychovisual redundancies. This is based on the imperfection and characteristics of Human Visual System.
  ...
  It's the spectral irrelevance.
  On top of that, Human Visual System is also less sensitive to high frequencies that represent small details, that's why a DCT (Discrete Cosine Transform) transform is applied, allowing to separate low and high frequencies.
  Then, a quantization is made, removing the high frequencies, and Human Visual System will not see the difference.
  It's the spatial irrelevance.
...
![Histogram for $QF_1 = QF_2$](./images/id_hist.png){#fig-id_hist fig-pos="H"}

![Pairwise analysis for $QF_1 = QF_2$](./images/id_pair.png){#fig-id_pair fig-pos="H"}
...
In the case where $QF_1 = QF_2$, the double JPEG compression doesn't leave any trace, as it's possible to see on figures [-@fig-id_hist] and [-@fig-id_pair].
...
In this way, the goal is to determine if the image has been modified, to avoid trusting an image that is not real.
...
In case of double compression of the right part of the image with $QF_1 > QF_2$, the histogram contains periodicaly some bins that are bigger than their neighbors.
This behavior is once again not normal which allows, in the same way as for the image manipulated in the DCT domain with $QF_1 < QF_2$ to determine that the image has been modified.
On top of that, due to histogram structure, it's possible to determine that the image has undergone a double compression with $QF_1 > QF_2$ because the histogram is of this type, as explained in section [-@sec-double2].

Finaly, in case of double compression using $QF_1 < QF_2$, the histogram of DCT coefficient is clearly not normal because there are a lot of holes between bins,
like for the double compression with $QF_1 < QF_2$ explained in section [-@sec-double].
So in this case, we can easily determinate that the image has undergone double compression with $QF_1 < QF_2$, thanks to the presence of holes.
...
The JPEG compression is based on redundancies, both spatial and psychovisual.
It allows to reduce the amount of data needed to store a photo whith a loss of informations.
During compression, quantization is performed, leaving some artifacts when double compressing with different quality factors.
These artifacts can be analysed and detected by studying the DCT domain, more specifically by studying the DCT coefficients.

When the quality of first compression is bigger than the quality of second compression, there are periodicaly some bins that are greater than their neighbors.
And when the quality of first compression is less than the quality of second compression, there are holes that appears between bins in the histogram of DCT coefficients.
...
However, it's not possible to detect when image is composed of two images merged in DCT domain with image compressed 2 times with $QF_1 > QF_2$,
or when double compression is made with exactly the same quality factor.

So in this way, it's possible to detect if an image is modified depending on the modification of the image and of the quality factors.
However, if image is not detected as having been modified, this does not mean that the image has not been altered.
That is why other methods must be used before concluding that the image has not been altered.
