---
title: "Digital Forensics: CC2 Review Guide"
subtitle: "Comprehensive Answers & Analysis"
format:
  html:
    toc: true
    toc-depth: 2
...
# Digital Forensics Exam Preparation

This document provides detailed answers to the CC2 questions based on the course material. Use the tabs below each question to switch between **French** and **English**.
...
The active approach involves **inserting information** into the document or the printing process to aid authentication.
...
* **Détection (Attaque PoV) :** L'analyse de l'histogramme révèle une anomalie. Les paires de valeurs (ex: $2k$ et $2k+1$) tendent à s'égaliser en fréquence ($h(0) \approx h(1)$).
* **Contre-mesure (LSB Matching / +/-1 steg) :** Au lieu de remplacer brutalement le bit, on ajoute ou soustrait 1 aléatoirement pour correspondre au bit cible. Cela lisse l'histogramme et rend la détection plus difficile.
...
* **Detection (PoV Attack):** Histogram analysis reveals an anomaly. Pairs of values (e.g., $2k$ and $2k+1$) tend to equalize in frequency ($h(0) \approx h(1)$).
* **Countermeasure (LSB Matching / +/-1 steg):** Instead of brutally replacing the bit, one randomly adds or subtracts 1 to match the target bit. This smooths the histogram and makes detection harder.
...
* **Voice:** Replay of recordings, speech synthesis, voice conversion.
